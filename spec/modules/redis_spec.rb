...
describe Sidekiq::Grouping::Redis do
  ...
  describe "#push_msg" do
    it "adds message to queue" do
      ...
      expect(redis { |c| c.call 'LLEN', key }).to eq 1
      expect(redis { |c| c.call 'LRANGE', key, 0, 1 }).to eq ['My message']
      expect(redis { |c| c.call 'SMEMBERS', unique_key}).to eq []
    end

    it "remembers unique message if specified" do
      subject.push_msg(queue_name, 'My message', true)
      expect(redis { |c| c.call 'SMEMBERS', unique_key}).to eq ['My message']
    end
  end
